...
- 📝 **全功能歌词缓存管理与 LRC 文本编辑器**  
  在设置界面可随时浏览已缓存曲库（显示歌名、歌手、来源平台与缓存时间），支持按歌名/歌手实时搜索过滤、单曲快捷删除与一键全量清空；支持进入详情页直接编辑修改原始 LRC 歌词文本并保存生效（标记为自定义编辑），亦支持单曲一键在线三源重新下载覆盖。
...
- **本地缓存管理**：
  ...
  - 点击列表条目进入**歌词详情页**：可直接查阅与编辑修改原始 LRC 文本并保存，亦可点击「在线重下」强制从云端重新拉取最新歌词；
