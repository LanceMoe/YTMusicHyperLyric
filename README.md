...
- **车载蓝牙歌词总控**：随时开启/关闭车载蓝牙歌词功能；
- **仅连接蓝牙时生效** (默认开启)：未连蓝牙时不更改手机锁屏/通知栏歌名，连接汽车时自动推送歌词；
- **显示槽位选择**：支持**标题栏替换** (推荐)、**歌名 - 歌词拼接**、**歌手栏替换** 或 **专辑栏替换**；
- **时间轴补偿**：支持微调毫秒偏移，完美匹配不同品牌车机的蓝牙音频解码延迟；
- **本地缓存管理**：
  - 概览本地歌曲记录总数，包含自动下载失败的歌曲；
  - 按歌名或歌手实时搜索过滤缓存列表；
  - 快速删除单首歌曲缓存，或一键清空全部本地歌词；
  - 点击列表条目进入**歌词详情页**：可修改歌名关键词和歌手（可留空）后点击「搜索歌词」，获取的歌词先填入编辑框，编辑后点击「保存修改」关联到原歌曲；亦可直接粘贴 LRC 文本或点击「在线重下」重新获取并保存；
  - 下载失败的记录不会阻止下次播放时重试，也不会覆盖已有歌词。更新前未保存的历史失败歌曲，需要再次播放并检索后才会出现在列表中；
- **即时测试工具**：输入歌名与歌手即可一键测试三源抓取、自动保存至缓存并预览车机推送效果。
